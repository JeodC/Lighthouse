...
BK's skinning is **rigid**: there are no vertex weights, and each *vertex* follows exactly one bone ([`rendernormal.c:30`](../../src/core2/model/rendernormal.c#L30)). That function is the whole of it: one matrix per entry, applied to a rest position, written into every vertex that shares it. A seam closes by a triangle's corners following different bones, not by blending, so a "bend" is really mesh segments riding a chain of bones. Joints go on natural geometry rings so the seams hide.
